...
export class GeminiSummarizationProvider implements SummarizationProvider {
  ...
  async generateCandidateSummary(
    ...
    try {
      ...
      // Validation
      const validatedInstance = plainToInstance(ValidatedSummaryResult, parsedResult);
      const errors = await validate(validatedInstance);

      if (errors.length > 0) {
        this.logger.error(`Validation failed for Gemini output: ${JSON.stringify(errors)}`);
        throw new Error('Invalid structure in LLM response');
      }

      return validatedInstance as CandidateSummaryResult;
    } catch (error: any) {
      this.logger.error(`Error in GeminiSummarizationProvider: ${error.message}`);
      throw error;
    }
  }
}
